...
// we can replace blocks in here in 'child' template files
...
<head>
	...
	<title><?= $millco_title; ?></title>

	<?php

	// metatags include has CSP etc.
	include('includes/metatags.php');

	// list of css files.
	$styles_files = array(
		"css/00_millco_base.css",
		"css/10_layout.css",
		"css/20_typography.css",
		"css/30_nav.css",
	...
	if ($page->template == 'shop') {
		$styles_files[] = "css/110_comments.css";
	}

	// If we're using ProCache then this isn't really needed but won't do any harm.
	foreach ($styles_files as $sf) {
		if ($config->debug) {
			$cache_time = time();
		} else {
			$cache_time = 1;
		}
		echo '<link rel="stylesheet" href="/site/templates/' . $sf . '?v=' . $cache_time . '">' . PHP_EOL;
	}

	echo '<link rel="stylesheet" href="/site/assets/vendor/photoswipe/photoswipe.css">';

	// include open graph tags
	include('includes/og_tags.php');
	...
	?>
		...
		<style nonce="<?= $nonce ?>">
			.body_bg {
				background-color: hsl(<?= $hue ?>, 92%, 42%) !important;
			}

			.logo_fill {
				fill: hsl(<?= $hue ?>, 92%, 42%) !important;
			}
		</style>
	<?php
	}
...
<body class="body_bg <?= $body_classes ?>">
	...
	<div id="app" class="<?= $app_classes ?>">
					...
					<div class="logo logo_newt">
						<svg viewBox="0 0 624 324" xmlns="http://www.w3.org/2000/svg">
							...
							<g fill="#fff">
								...
								<path d="m437.52 181.92h6.02l.25 3.04c.4-.49.84-.94 1.31-1.34s.98-.75 1.53-1.04c.71-.38 1.49-.68 2.34-.89s1.76-.31 2.75-.31c1.89 0 3.57.37 5.02 1.09 1.45.73 2.67 1.76 3.65 3.09s1.73 2.92 2.23 4.77.75 3.89.75 6.11v.57c0 2.13-.25 4.12-.75 5.96s-1.24 3.43-2.23 4.76c-.98 1.35-2.2 2.41-3.64 3.17-1.44.77-3.09 1.15-4.95 1.15-1.06 0-2.03-.11-2.91-.33s-1.7-.54-2.45-.96c-.44-.25-.86-.55-1.26-.88s-.78-.68-1.12-1.07v14.05h-6.54v-40.96zm18.91 10.76c-.27-1.18-.67-2.2-1.22-3.08-.57-.88-1.29-1.57-2.16-2.08-.88-.51-1.92-.77-3.14-.77-.68 0-1.3.07-1.89.22-.58.15-1.1.36-1.56.63-.53.29-.99.66-1.38 1.09-.39.44-.73.93-1.03 1.48v13.15c.25.51.56.98.93 1.41.36.43.78.79 1.26 1.08.49.29 1.05.52 1.67.7.62.17 1.31.26 2.08.26 1.2 0 2.24-.26 3.1-.78.87-.52 1.58-1.23 2.15-2.12.55-.91.95-1.96 1.2-3.13.25-1.18.38-2.42.38-3.73v-.57c0-1.33-.13-2.58-.4-3.76z" />
								<path d="m343.32 259.34-2.71 9.16h-6.84l12.96-39.81h5.82l12.74 39.81h-6.81l-2.68-9.16h-12.5zm10.88-5.63-4.57-15.75-4.65 15.75h9.21z" />
								<path d="m392.66 238.59c.99.15 1.77.32 2.34.52l-.93 6.45c-1-.22-1.99-.38-2.97-.49s-1.95-.16-2.91-.16c-1.09 0-2.08.11-2.95.33s-1.65.56-2.32 1.01c-.73.44-1.35.99-1.86 1.67-.51.67-.94 1.45-1.29 2.32v18.27h-6.54v-29.59h6.12l.27 4.1.03.68c1.2-1.66 2.65-2.96 4.33-3.91 1.69-.95 3.55-1.42 5.59-1.42 1.06 0 2.08.07 3.08.22z" />
								<path d="m415.74 263.43c.76-.25 1.43-.6 2.01-1.05.56-.46 1.02-.99 1.35-1.61.34-.62.51-1.3.51-2.05h6.15c.02 1.44-.3 2.79-.97 4.06s-1.56 2.36-2.69 3.27c-1.15.95-2.46 1.69-3.95 2.23s-3.05.81-4.69.81c-2.28 0-4.27-.4-5.97-1.19s-3.12-1.86-4.25-3.21c-1.15-1.35-2.01-2.92-2.6-4.72-.58-1.79-.88-3.71-.88-5.76v-.96c0-2.02.29-3.94.86-5.74s1.44-3.38 2.58-4.73c1.15-1.35 2.57-2.42 4.28-3.21s3.69-1.19 5.95-1.19c1.79 0 3.44.27 4.96.82s2.83 1.31 3.92 2.3 1.95 2.16 2.56 3.53.91 2.87.89 4.51h-6.15c0-.8-.15-1.56-.45-2.27s-.72-1.33-1.27-1.86c-.56-.53-1.23-.95-2.01-1.26s-1.61-.46-2.5-.46c-1.37 0-2.5.28-3.4.83-.9.56-1.62 1.28-2.15 2.17-.56.89-.96 1.91-1.19 3.06s-.34 2.32-.34 3.5v.96c0 1.2.11 2.38.34 3.54s.62 2.17 1.16 3.05c.53.91 1.25 1.64 2.16 2.19s2.05.82 3.42.82c.82 0 1.61-.12 2.37-.37z" />
								<path d="m441.92 241.51c.47-.49.97-.92 1.5-1.29.82-.6 1.73-1.06 2.72-1.38s2.05-.48 3.16-.48c1.55 0 2.97.23 4.25.68 1.29.46 2.38 1.17 3.29 2.13.91.97 1.62 2.2 2.12 3.71s.75 3.3.75 5.4v18.21h-6.54v-18.27c0-1.09-.13-2.05-.4-2.87s-.65-1.49-1.16-2c-.53-.55-1.17-.95-1.93-1.22-.76-.26-1.63-.4-2.61-.4-.77 0-1.49.09-2.17.27s-1.31.45-1.87.79c-.47.27-.92.6-1.33.98s-.77.81-1.08 1.29v21.41h-6.54v-42h6.54v16.62c.38-.58.81-1.12 1.29-1.61z" />
								<path d="m484.35 238.92v24.17h8.7v5.41h-24.39v-5.41h9.11v-18.73h-9.11v-5.44h15.7zm-6.92-9.12c.18-.45.45-.83.79-1.16.33-.31.72-.55 1.19-.72.46-.17.99-.26 1.57-.26.98 0 1.78.25 2.39.75s.96 1.14 1.05 1.93c.13.8 0 1.5-.41 2.09-.4.59-.98.99-1.75 1.19-.38.35-.84.57-1.37.68s-1.11.06-1.75-.14c-.66-.2-1.15-.56-1.49-1.08s-.51-1.12-.51-1.82c0-.53.09-1.02.27-1.46z" />
								<path d="m495.99 238.92h6.78l7.33 21.46.44 1.97.44-1.97 7.25-21.46h6.78l-11.59 29.59h-5.77l-11.65-29.59z" />
								<path d="m538.25 267.96c-1.81-.73-3.37-1.74-4.66-3.04-1.29-1.28-2.29-2.78-2.99-4.53-.7-1.74-1.05-3.62-1.05-5.65v-1.12c0-2.31.36-4.41 1.09-6.29s1.73-3.48 3.01-4.81 2.76-2.36 4.44-3.08c1.69-.72 3.49-1.08 5.4-1.08 2.13 0 4.02.36 5.66 1.08s3.02 1.72 4.13 2.99c1.11 1.29 1.95 2.83 2.52 4.62.56 1.79.85 3.74.85 5.85v2.82h-20.51v.14c.2 1.29.51 2.38.93 3.25.42.88.99 1.66 1.72 2.35.73.73 1.59 1.29 2.58 1.7.99.4 2.08.6 3.27.6 1.62 0 3.13-.31 4.54-.94 1.4-.63 2.56-1.53 3.47-2.69l3.5 3.39c-.98 1.42-2.49 2.7-4.53 3.83-2.03 1.13-4.49 1.7-7.37 1.7-2.19 0-4.19-.37-6-1.09zm2.62-23.78c-.79.34-1.5.82-2.12 1.44-.62.64-1.14 1.4-1.56 2.3-.42.89-.73 1.91-.93 3.04h13.92v-.44c0-.86-.16-1.71-.46-2.56-.31-.85-.75-1.59-1.31-2.23-.55-.62-1.24-1.12-2.06-1.49-.83-.37-1.79-.56-2.88-.56-.93 0-1.79.17-2.58.51z" />
							</g>
							<path d="m159 35c-70.14 0-127 56.86-127 127s56.86 127 127 127 127-56.86 127-127-56.86-127-127-127zm0 139.17c-6.72 0-12.18-5.45-12.18-12.18s5.45-12.18 12.18-12.18 12.18 5.45 12.18 12.18-5.45 12.18-12.18 12.18z" fill="#666" />
							<path d="m154.92 173.5-36.03 101.66-2.66 7.52c-18.14-6.43-34.41-16.84-47.74-30.17s-23.74-29.6-30.17-47.75l7.51-2.66 101.66-36.02c1.23 3.45 3.97 6.2 7.43 7.42z" fill="#aaa" />
							<path d="m279.68 119.23-7.52 2.66-101.66 36.03c-1.22-3.46-3.96-6.2-7.42-7.43l36.02-101.66 2.66-7.51c17.65 6.25 33.53 16.27 46.66 29.09l2.17 2.17c12.81 13.13 22.83 29 29.09 46.65z" fill="#aaa" />
							<path d="m279.68 119.23c-6.26-17.65-16.28-33.52-29.09-46.65l-2.17-2.17c-13.13-12.82-29.01-22.84-46.66-29.09-13.37-4.74-27.77-7.32-42.76-7.32-70.7 0-128 57.3-128 128 0 14.99 2.58 29.39 7.32 42.76 6.43 18.15 16.83 34.41 30.17 47.75 13.33 13.33 29.6 23.74 47.74 30.17 13.37 4.74 27.77 7.32 42.77 7.32 70.69 0 128-57.31 128-128 0-15-2.58-29.4-7.32-42.77zm-35.83 127.62c-22.67 22.66-52.8 35.15-84.85 35.15-13.88 0-27.4-2.34-40.11-6.84-16.64-5.88-31.9-15.46-44.75-28.31s-22.42-28.11-28.31-44.75c-4.49-12.71-6.83-26.23-6.83-40.1 0-32.06 12.48-62.19 35.14-84.86 22.67-22.66 52.8-35.14 84.86-35.14 13.87 0 27.39 2.34 40.1 6.83 16.64 5.89 31.9 15.46 44.75 28.31s22.43 28.11 28.31 44.75c4.5 12.71 6.84 26.23 6.84 40.11 0 32.05-12.49 62.18-35.15 84.85z" />
							<path d="m159 58c57.35 0 104 46.65 104 104s-46.65 104-104 104-104-46.65-104-104 46.65-104 104-104zm0-8c-61.86 0-112 50.14-112 112s50.14 112 112 112 112-50.14 112-112-50.14-112-112-112z" />
							<path d="m159 74c48.52 0 88 39.48 88 88s-39.48 88-88 88-88-39.48-88-88 39.48-88 88-88zm0-8c-53.02 0-96 42.98-96 96s42.98 96 96 96 96-42.98 96-96-42.98-96-96-96z" />
							<path d="m159 90c39.7 0 72 32.3 72 72s-32.3 72-72 72-72-32.3-72-72 32.3-72 72-72zm0-8c-44.18 0-80 35.82-80 80s35.82 80 80 80 80-35.82 80-80-35.82-80-80-80z" />
							<path d="m159 106c30.88 0 56 25.12 56 56s-25.12 56-56 56-56-25.12-56-56 25.12-56 56-56zm0-8c-35.35 0-64 28.65-64 64s28.65 64 64 64 64-28.65 64-64-28.65-64-64-64z" />
							<path d="m159 106c-30.93 0-56 25.07-56 56s25.07 56 56 56 56-25.07 56-56-25.07-56-56-56zm0 63c-3.87 0-7-3.13-7-7s3.13-7 7-7 7 3.13 7 7-3.13 7-7 7z" class="logo_fill" />
						</svg>
					</div>
				</a>
			</div>
			<div class="header_search">
				<form action="/search/" id="search-form" class="search-form">
					...
					<div class="search_box_layout">
						...
						//<input type="text" id="search-box" class="search-box" name="q" placeholder="" value="" required="">
						?>
						...
						//  echo '<input type="text" id="search-box" class="search-box" name="q" placeholder="" value="" required="">';
		...
		<main id="millco_layout" class="container">
			...
			<div id="millco_main">
				<div class="shop_grid">
					<div id="content_main" class="shop_main">
					...
					<div id="content_aside" class="shop_aside shop_1 pt-2 pb-2">
						<?php
			...
			<div id='pw-optional' pw-optional></div>
	</div>
	</main>
